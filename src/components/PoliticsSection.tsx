...
export default function PoliticsSection({ articles, onArticleClick }: PoliticsSectionProps) {
  const politicsArticles = articles.filter(a => a.category === 'politics' && !a.isDraft);
  const featuredArticle = politicsArticles[0] || null;
  const listArticles = politicsArticles.slice(1);
    ...
    <div className="space-y-10 font-sans">
      ...
      <div className="border-b border-stone-200 pb-4 select-none">
        ...
        <p className="text-xs text-stone-550 font-mono mt-1">
          In-depth reports, legislative summaries, and regional policy development coverage.
        </p>
      </div>
      ...
      {politicsArticles.length === 0 ? (
        <div className="p-12 border border-dashed border-stone-250 rounded-2xl text-center font-mono text-xs text-stone-500 select-none">
          No politics articles configured yet. Add them in the Editor panel.
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          
          {/* Main Headline Story (Lefthand Side) */}
          {featuredArticle && (
            <div className="lg:col-span-7 space-y-5">
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-3 text-xs font-mono text-stone-500">
                  <span className="text-red-655 font-bold uppercase tracking-wider flex items-center gap-1">
                    <Newspaper className="w-3.5 h-3.5" /> Featured Analysis
                  </span>
                  <span>•</span>
                ...
                <h3 
                  ...
                  className="font-serif font-black text-xl sm:text-2xl md:text-3xl text-stone-900 leading-tight hover:text-red-700 cursor-pointer transition duration-150"
                >
              ...
              >
                <BookOpen className="w-3.5 h-3.5" /> Read Full Coverage
              </button>
            </div>
          )}

          {/* Directory Listings (Righthand Side) */}
