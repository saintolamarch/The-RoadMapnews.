...
export default function EducationSection({ articles, onArticleClick }: EducationSectionProps) {
  const eduArticles = articles.filter(a => a.category === 'education' && !a.isDraft);
  const featuredEdu = eduArticles[0];
  const secondaryEdu = eduArticles.slice(1);
    ...
    <div className="space-y-10 font-sans">
      ...
      <div className="border-b border-stone-200 pb-4 select-none">
        ...
        <p className="text-xs text-stone-550 font-mono mt-1">
          Academic reform, secondary school curriculum developments, and educational coverage.
        </p>
      </div>
      ...
      {!featuredEdu ? (
        <div className="p-12 border border-dashed border-stone-250 rounded-2xl text-center font-mono text-xs text-stone-500 select-none">
          No education articles available.
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-7 space-y-5">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-3 text-xs font-mono text-stone-500">
                <span className="text-red-655 font-bold uppercase tracking-wider flex items-center gap-1">
                  <Newspaper className="w-3.5 h-3.5" /> Education Reporter
                </span>
                <span>•</span>
                <span className="flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {featuredEdu.location}
                </span>
                <span>•</span>
                <span className="flex items-center gap-1">
                  <Calendar className="w-3 h-3" /> {featuredEdu.date}
                </span>
              </div>

              <h3 
                onClick={() => onArticleClick(featuredEdu)}
            ...
            {featuredEdu.image && (
              ...
              >
                <img 
                  src={featuredEdu.image} 
                  alt={featuredEdu.headline} 
            ...
            <p className="text-sm text-stone-850 leading-relaxed text-justify">
              {featuredEdu.snippet} {featuredEdu.content ? featuredEdu.content.substring(0, 180) + "..." : ""}
            </p>
            ...
            >
              <BookOpen className="w-3.5 h-3.5" /> Read Full Bulletin
            </button>
          </div>

          {/* List of Remaining Education Articles */}
              ...
              <div className="space-y-6 divide-y divide-stone-150">
                {secondaryEdu.map((art, idx) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-stone-655 line-clamp-2 leading-relaxed">
                      {art.snippet}
                    </p>
